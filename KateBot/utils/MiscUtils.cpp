...
bool CMiscUtils::CheckCSGOWindowState()
{
	...
	csgoActiveWindow = !strcmp(windowTitle, "Counter-Strike: Global Offensive - Direct3D 9" );
	return csgoActiveWindow;
}

bool CMiscUtils::DoesCSGOExist()
{
	HWND hwnd = FindWindow(NULL, "Counter-Strike: Global Offensive - Direct3D 9" );
	return (hwnd != NULL);
}

HWND CMiscUtils::GetGameWindow()
{
	return FindWindow(NULL, "Counter-Strike: Global Offensive - Direct3D 9" );
}
